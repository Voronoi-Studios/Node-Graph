...
</div>`;var g=class{constructor({wrapEl:t,walklinkEl:e,stepperEl:s,prevBtn:r,nextBtn:i,stepcountEl:a,steptextEl:n,canStep:l,updateCam:h}){this._wrapEl=t,this._walklinkEl=e,this._stepperEl=s,this._prevBtn=r,this._nextBtn=i,this._stepcountEl=a,this._steptextEl=n,this._canStep=l,this._updateCam=h,this.steps=[],this.mode="none",this.currentStep=0,this._allIds=[],this._walklinkEl.addEventListener("click",()=>this._startWalkthrough()),this._prevBtn.addEventListener("click",()=>this._goStep(-1)),this._nextBtn.addEventListener("click",()=>this._goStep(1))}init(t,e,s=[]){this.steps=t,this._allIds=s,t.length?e===0?(this.mode="whole",this._clearDim(),this._updateCam(this._allIds)):(this.mode="stepping",this.currentStep=Math.min(Math.max(e-1,0),t.length-1),this._applyDim(t[this.currentStep].ids),this._updateCam(t[this.currentStep].ids)):(this.mode="none",this._clearDim(),this._updateCam(this._allIds)),this._syncUI()}_applyDim(t){this._wrapEl.querySelectorAll(".ng-node").forEach(e=>{e.classList.toggle("ng-dim",!t.includes(e.dataset.nodeId))}),this._wrapEl.querySelectorAll(".ng-edge").forEach(e=>{let s=t.includes(e.dataset.from)&&t.includes(e.dataset.to);e.style.opacity=s?"1":"0.25"})}_clearDim(){this._wrapEl.querySelectorAll(".ng-node").forEach(t=>t.classList.remove("ng-dim")),this._wrapEl.querySelectorAll(".ng-edge").forEach(t=>t.style.opacity="1")}_syncUI(){if(this.mode==="none"){this._walklinkEl.hidden=!0,this._stepperEl.hidden=!0;return}if(this.mode==="whole"){let s=this._canStep();this._walklinkEl.hidden=!s,this._stepperEl.hidden=!0;return}this._walklinkEl.hidden=!0,this._stepperEl.hidden=!1;let t=this.steps[this.currentStep];this._stepcountEl.textContent=`step ${this.currentStep+1} of ${this.steps.length}`,this._steptextEl.textContent=t.label||"";let e=this._canStep();this._prevBtn.disabled=!e,this._nextBtn.disabled=!e||this.currentStep===this.steps.length-1}_startWalkthrough(){this.mode="stepping",this.currentStep=0,this._applyDim(this.steps[0].ids),this._updateCam(this.steps[0].ids),this._syncUI()}_exitToWhole(){this.mode="whole",this._clearDim(),this._updateCam(this._allIds),this._syncUI()}_goStep(t){let e=this.currentStep+t;if(e<0){this._exitToWhole();return}e>this.steps.length-1||(this.currentStep=e,this._applyDim(this.steps[this.currentStep].ids),this._updateCam(this.steps[this.currentStep].ids),this._syncUI())}};function w(p,t,e,s){let r=e.map(c=>t[c]).filter(Boolean);if(!r.length)return null;let i=1/0,a=1/0,n=-1/0,l=-1/0;r.forEach(c=>{let y=c.pos.x+s.offsetX,f=c.pos.y+s.offsetY;i=Math.min(i,y),a=Math.min(a,f),n=Math.max(n,y+(c.width||160)),l=Math.max(l,f+(c.height||90))});let h=60;i-=h,a-=h,n+=h,l+=h;let o=n-i,m=l-a,u=p.clientWidth,x=p.clientHeight;if(!o||!m||!u||!x)return null;let d=Math.min(u/o,x/m);d=Math.min(Math.max(d,.25),1.5);let E=(i+n)/2,C=(a+l)/2;return{x:u/2-E*d,y:x/2-C*d,scale:d}}function S(p,t){t&&(p.style.transform=`translate(${t.x}px, ${t.y}px) scale(${t.scale})`)}var I={"chevron-left":'<polyline points="15 18 9 12 15 6"/>',"chevron-right":'<polyline points="9 18 15 12 9 6"/>',play:'<polygon points="6 3 20 12 6 21 6 3"/>',"maximize-2":`
