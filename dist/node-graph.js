...
</div>`;function b(a,t,e){let{offsetX:s,offsetY:o,canvasW:i,canvasH:n}=e,l=[`<svg width="${i}" height="${n}" style="position:absolute;top:0;left:0;pointer-events:none" xmlns="http://www.w3.org/2000/svg"><g stroke="#f0f0f0" stroke-width="3" fill="none">`];return a.forEach(r=>{let p=t[r.from],h=t[r.to];if(!p||!h)return;let u=p.pos.x+s+p.width+11,g=p.pos.y+o+p.outputs[r.via],d=h.pos.x+s+10,c=h.pos.y+o+h.inputs["input-0"],_=Math.abs(d-u)*.2;l.push(`<path class="ng-edge" data-from="${r.from}" data-to="${r.to}" d="M${u} ${g} C${u+_} ${g} ${d-_} ${c} ${d} ${c}"/>`)}),l.push("</g></svg>"),l.join("")}var x=class{constructor({wrapEl:t,walklinkEl:e,stepperEl:s,prevBtn:o,nextBtn:i,stepcountEl:n,steptextEl:l,canStep:r,updateCam:p}){this._wrapEl=t,this._walklinkEl=e,this._stepperEl=s,this._prevBtn=o,this._nextBtn=i,this._stepcountEl=n,this._steptextEl=l,this._canStep=r,this._updateCam=p,this.steps=[],this.mode="none",this.currentStep=0,this._allIds=[],this._walklinkEl.addEventListener("click",()=>this._startWalkthrough()),this._prevBtn.addEventListener("click",()=>this._goStep(-1)),this._nextBtn.addEventListener("click",()=>this._goStep(1))}init(t,e,s=[]){this.steps=t,this._allIds=s,t.length?e===0?(this.mode="whole",this._clearDim(),this._updateCam(this._allIds)):(this.mode="stepping",this.currentStep=Math.min(Math.max(e-1,0),t.length-1),this._applyDim(t[this.currentStep].ids),this._updateCam(t[this.currentStep].ids)):(this.mode="none",this._clearDim(),this._updateCam(this._allIds)),this._syncUI()}_applyDim(t){this._wrapEl.querySelectorAll(".ng-node").forEach(e=>{e.classList.toggle("ng-dim",!t.includes(e.dataset.nodeId))}),this._wrapEl.querySelectorAll(".ng-edge").forEach(e=>{let s=t.includes(e.dataset.from)&&t.includes(e.dataset.to);e.style.opacity=s?"1":"0.25"})}_clearDim(){this._wrapEl.querySelectorAll(".ng-node").forEach(t=>t.classList.remove("ng-dim")),this._wrapEl.querySelectorAll(".ng-edge").forEach(t=>t.style.opacity="1")}_syncUI(){if(this.mode==="none"){this._walklinkEl.hidden=!0,this._stepperEl.hidden=!0;return}if(this.mode==="whole"){let s=this._canStep();this._walklinkEl.hidden=!s,this._stepperEl.hidden=!0;return}this._walklinkEl.hidden=!0,this._stepperEl.hidden=!1;let t=this.steps[this.currentStep];this._stepcountEl.textContent=`step ${this.currentStep+1} of ${this.steps.length}`,this._steptextEl.textContent=t.label||"";let e=this._canStep();this._prevBtn.disabled=!e,this._nextBtn.disabled=!e||this.currentStep===this.steps.length-1}_startWalkthrough(){this.mode="stepping",this.currentStep=0,this._applyDim(this.steps[0].ids),this._updateCam(this.steps[0].ids),this._syncUI()}_exitToWhole(){this.mode="whole",this._clearDim(),this._updateCam(this._allIds),this._syncUI()}_goStep(t){let e=this.currentStep+t;if(e<0){this._exitToWhole();return}e>this.steps.length-1||(this.currentStep=e,this._applyDim(this.steps[this.currentStep].ids),this._updateCam(this.steps[this.currentStep].ids),this._syncUI())}};function w(a){let t=Object.values(a);if(!t.length)return{minX:0,minY:0,maxX:0,maxY:0};let e=1/0,s=1/0,o=-1/0,i=-1/0;return t.forEach(n=>{e=Math.min(e,n.pos.x),s=Math.min(s,n.pos.y),o=Math.max(o,n.pos.x+(n.width||160)),i=Math.max(i,n.pos.y+(n.height||90))}),{minX:e,minY:s,maxX:o,maxY:i}}function S(a,t,e,s){let o=Object.fromEntries(e.map(f=>[f,t[f]]).filter(([,f])=>f)),{minX:i,minY:n,maxX:l,maxY:r}=w(o),p=60;i+=s.offsetX-p,n+=s.offsetY-p,l+=s.offsetX+p,r+=s.offsetY+p;let h=l-i,u=r-n,g=a.clientWidth,d=a.clientHeight;if(!h||!u||!g||!d)return null;let c=Math.min(g/h,d/u);c=Math.min(Math.max(c,.25),1.5);let _=(i+l)/2,$=(n+r)/2;return{x:g/2-_*c,y:d/2-$*c,scale:c}}function k(a,t){t&&(a.style.transform=`translate(${t.x}px, ${t.y}px) scale(${t.scale})`)}function E(a){let{minX:t,minY:e,maxX:s,maxY:o}=w(a),i=60;return t-=i,e-=i,s+=i,o+=i,{offsetX:-t,offsetY:-e,canvasW:s-t,canvasH:o-e}}var L={"chevron-left":'<polyline points="15 18 9 12 15 6"/>',"chevron-right":'<polyline points="9 18 15 12 9 6"/>',play:'<polygon points="6 3 20 12 6 21 6 3"/>',"maximize-2":`
